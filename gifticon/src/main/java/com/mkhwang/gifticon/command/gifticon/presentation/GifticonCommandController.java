...
import com.mkhwang.gifticon.command.gifticon.application.GifticonCommandHandler;
...
public class GifticonCommandController {
  ...
  public ResponseEntity<ApiResponse<GifticonDto.Gifticon>> tradeGifticon(@PathVariable Long id, @RequestBody GifticonTradeRequest request) {
    GifticonDto.Gifticon gifticon = gifticonCommandHandler.tradeGifticon(gifticonCommandMapper.toTradeCommand(id, request.getUserId()));
  ...
  public ResponseEntity<?> deleteGifticon(@PathVariable Long id) {
    gifticonCommandHandler.deleteGifticon(gifticonCommandMapper.toDeleteCommand(id));
